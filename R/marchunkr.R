...
#' @description Addin that splits full documents into Rmarkdown margin chunks or
#' wraps highlighted text in an active RStudio document editor in a Rmarkdown margin chunk.
#' @return NULL
#' @details
#'
...
marchunkr_section <- function() {
  ...
  if (!remedy_opts$get("basic")) {
    if (remedy_opts$get("counter")) {
      chunky_num <- length(grep(sprintf("\\`\\`\\`\\{marginfigure %s", remedy_opts$get("name")), adc$contents[1:adc$selection[[1]]$range$start[[1]]])) + 1
